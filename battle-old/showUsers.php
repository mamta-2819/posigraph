...
function usersToMe()
{
    ...
    if($requestedUser)
    {
        if(mysqli_num_rows($requestedUser) >= 1)
        {
            while($row=mysqli_fetch_array($requestedUser))
            {
                 echo $query="select firstName ,dp from user where userId={$row['senderId']}";
                  $nameDp=mysqli_query($conn,$query);
                        ...
                        <div class='col-sm-12 user-detail'>
                              ...
                              <div class=' user-name-buttons'> 
                                 ...
                                 <div class='row btn'> <a href='#'>
                                  <button data-id='{$friend['userId']}'  data-name='{$friend['firstName']}' class='btn btn-success unfriend-btn'>Start Battle</button></a>
                                  </div>
